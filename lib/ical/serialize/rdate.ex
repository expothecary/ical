defmodule ICal.Serialize.Rdate do
  ...
  def property(dates, acc) do
    ...
    dates
    |> Enum.reduce(%{}, &by_tz/2)
    |> Enum.reduce(acc, &to_rdate_ics/2)
  end
  ...
  defp by_tz(%NaiveDateTime{} = date, acc) do
    serialized = [Serialize.value(date)]

    Map.update(acc, :floating_datetime, serialized, fn dates ->
      dates ++ serialized
    end)
  end
  ...
  defp to_rdate_ics({:floating_datetime, periods}, acc) do
    acc ++ ["RDATE:", Enum.intersperse(periods, ?,), ?\n]
  end
